<template>
  <div class="lns-record-list">
    ...
    <div v-if="scalanet_records.length > 0" class="records-group">
      <span class="record-type-title">{{
        $t("titles.lnsScalanetRecords")
      }}</span>
      <LNSRecordList
        :record-list="scalanet_records"
        :is-scalanet="true"
        @onUpdate="onUpdate"
        @onRenew="onRenew"
      />
...
import { session_name_or_scalanet_name } from "src/validators/common";
...
export default {
  ...
  computed: mapState({
    ...
    scalanet_records(state) {
      return this.records_of_type(state, "scalanet");
    },
    needsDecryption() {
      const records = [...this.scalanet_records, ...this.session_records];
