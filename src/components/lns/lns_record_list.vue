  ...
  <q-list link no-border :dark="theme == 'dark'" class="lns-record-list">
    ...
    >
      ...
      <q-item-section v-if="!isLocked(record)" side>
        ...
        <span v-else class="scalanet-expiration">{{
  ...
  methods: {
    ...
    copyNameI18nLabel(record) {
      if (record.type === "session") {
        ...
        return "menuItems.copyScalanetName";
      }
    },
    copyValueI18nLabel(record) {
      if (record.type === "session") {
        return "menuItems.copySessionId";
      } else if (record.type === "scalanet") {
        return "menuItems.copyScalanetAddress";
      }
      return "menuItems.copyAddress";
    },
    validMenuItems(record) {
      // change name depending on if scalanet or session
      const lockedItems = [
    ...
    copyValue(record) {
      let message = this.$t("notification.positive.scalanetAddressCopied");
      if (record.type === "session") {
