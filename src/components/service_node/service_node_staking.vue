...
export default {
  ...
  data() {
    return {
      service_node: {
        ...
        minStakeAmount: 0,
        maxStakeAmount: this.unlocked_balance / 1e2
      },
      confirmFields: {
        isBlink: false,
        totalAmount: -1,
  ...
  methods: {
    scalaWebsite() {
      const url = "https://scala.network/service-nodes/";
    ...
    maxStake() {
      const node = this.getNodeWithPubKey();
      return this.openForContributionScala(node);
    },
    getFeeDecimal(node) {
